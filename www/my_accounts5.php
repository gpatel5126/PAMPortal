<?php
	...
	if ( isset($_COOKIE['user_id']) ) {
		...
		$i = 0;//gets user id and sets it as $target.
	...
	$j = 0;//checks user id for server compliance and counts them.  
	if ($stmt = $mysqli->prepare("
		SELECT UID 
	...
	$accounts = 0;//check user id for account compliance, and counts them. 
	if ($stmt = $mysqli->prepare("
		SELECT UID 
	...
	if ($i != 0 && ($j != 0 || $accounts != 0)) {//i=user id, j=servers owned, accounts=accounts owned.
	...
	//Need the above data to calculate 'Owned' 'In Team' 'Unprotected' 'Compliance' numbers at the bottom of page. 
?>
<!DOCTYPE html>
<html>
<head>
	...
	$(function() {
		...
		$('.fancybox').fancybox({//calling class fancybox and using a fancybox method to size box. 
			autoSize: false,
		...
		var itemRow = $('table.team tr.s_row');//assigning var itemRow values. 
			...
			<div>
				...
				<div id="load_container">
					...
					<div id="help_window">
						...
						<p>
							In iaculis lorem lacus, eget mollis diam lobortis nec. Sed sit amet nulla vel ligula cursus pulvinar. Sed rutrum, dui eu faucibus iaculis, erat sapien molestie risus, vel volutpat odio quam nec diam.
						</p>
					</div>
