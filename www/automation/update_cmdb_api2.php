...
class AuthHeader {
 ...
 $action_result = $mysqli->query("
	SELECT DISTINCT server_actions.server_name, server_actions.user_id, server_actions.action_type, server_actions.transfer_to, server_actions.transfer_to_full_name, server_actions.date FROM server_actions WHERE DATE BETWEEN DATE( DATE_SUB( NOW() , INTERVAL 1 DAY ) ) AND DATE ( NOW() ) 
	");
	echo '<pre>';
	print_r ($action_result->fetch_assoc());
	echo '</pre>';
	die();
