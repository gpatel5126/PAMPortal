<?php
	...
	$extra = 0;
	// An array for all the people who now have 0 accounts
	$more_array = array();
	$mysqli->real_query("
		SELECT t1.uid
		FROM (SELECT * FROM account_user_compliance_history WHERE date = '$date_of_previous_update') t1
		LEFT JOIN (SELECT * FROM account_user_compliance_history WHERE date = '$date_of_current_update') t2 ON (t1.UID = t2.UID)
