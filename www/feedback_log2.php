...
<aside id="sidebar">
	...
	<nav>
		...
		<ul>
			...
			<?php if ($core) { ?><li><a href="feedback_log.php"class="active"><b></b><i class="fa fa-file-text"></i>Feedback Log</a></li> <?php } ?>
		</ul>
		<?php } ?>
...
<h1 id="main"><i class="fa fa-file-text"></i>Feedback Log
<?php if ($priv) { ?>
...
<div id="content">
	<div id="mydash">
		...
		<div class="one-col cf">
			<div>
				<h2><i class="fa fa-file-text"></i>Feedback Submitted by Users</h2>
				<table class="my_servers">
					...
					<?php 
						if ($stmt = $mysqli->prepare("
							SELECT * 
							FROM feedback
							LEFT JOIN ad ON ad.UID = feedback.username
						...
						")) {
							...
							while ($row = $res->fetch_assoc()) {
								...
								echo "<tr>";
									...
									<td>
									<form name="status" action="feedback_log.php" method="post">
									<select name="status">
						...
						if ($i == 0) {
							echo "<td></td>";
							echo "<td colspan='5'>No feedback submitted.</td>";
						}
