...
<aside id="sidebar">
	...
	<nav>
		...
		<ul>
			...
			<?php if ($core) { ?><li><a href="login_logs.php" class="active"><b></b><i class="fa fa-file-text"></i>Log of User Logins</a></li> <?php } ?>
...
<h1 id="main"><i class="fa fa-file-text"></i>Login Logs
<?php if ($priv) { ?>
...
<div id="content">
	<div id="mydash">
		...
		<div class="one-col cf">
			<div>
				<h2><i class="fa fa-file-text"></i>Log of User Logins Sorted by Date/Time</h2>
				<table class="my_servers">
					...
					<?php 
						// DATE_FORMAT(log_date, INTERVAL 8 HOUR)
						if ($stmt = $mysqli->prepare("
							SELECT login_logs.*, DATE_FORMAT(log_date,'%m/%d/%Y %r') AS niceDate, ad.FullName
							FROM login_logs
							LEFT JOIN ad ON ad.UID = login_logs.username
						...
						if ($i == 0) {
							echo "<td></td>";
							echo "<td colspan='5'>Nobody has logged in yet</td>";
						}
