...
// Load the initial list of safes
		...
		if (report == "not_available") {
			$("table#my_servers thead tr").append("<th class='action'>Action</th>");
		}
	...
	function loadServers(current_page) {
		$.ajax({ 
			...
			data: { report: report, server_query: search_value, page: current_page, prot: prot, roles: roles }, 
			dataType: 'json',
			success: function (data) { 					
				...
				if (data.num_results == 0) {
					...
					$.each(data.results, function(i, item) {
						...
						var lineItem = "<tr class='"+status_class+"'><td class='status'><span></span></td><td class='search'>"+item.address+"</td><td>"+item.owner+"</td><td>"+item.primary_contact+"</td><td>"+status_name+"</td><td>"+item.confidentiality+"</td><td>"+item.role+"</td>"
						if (report == "not_available") { lineItem += "<td><a class='claim' href='#' rel='{$row['CI Name']}'><i class='fa fa-chevron-circle-right'></i>Claim this server</a></td>"; }
						lineItem += "</tr>";
