<?php
	...
	$mysqli->real_query("
		SELECT implemented, COUNT(*) AS counted
		FROM cmdb_inventory
		WHERE cmdb_inventory.`System Role` IN ('Iaas Managed Blade','Iaas AIX LPAR Managed','Iaas Linux Managed VM','Iaas Windows Managed VM','Legacy Windows Managed Server')
		GROUP BY implemented
	");
	$res = $mysqli->use_result();
	while ($row = $res->fetch_assoc()) {
	...
	$mysqli->real_query("INSERT INTO total_history (`total_servers_y`,`total_servers_n`,`it_managed_y`,`it_managed_n`,`it_owned_y`,`it_owned_n`,`self_managed_y`,`self_managed_n`,`no_owner_y`,`no_owner_n`,`total_accounts_y`,`total_accounts_n`,`exceptions`,date)
